...
def select(doc):
    ...
    if len(usable) + len(flagged) + len(excluded) != len(per):
    ...
    for g in doc['regions']:
        ...
        per_region[str(g['region'])] = {'k_first': g['k_first'], 'k_last': g['k_last'], 'slices': len(rows),
                                        'usable': sum(1 for p in rows if p['identity_status'] in POLICY['usable']),
                                        'usable_flagged': sum(1 for p in rows if p['identity_status'] in POLICY['usable_flagged']),
                                        'excluded': sum(1 for p in rows if p['identity_status'] in POLICY['excluded'])}
    return {
        'policy': POLICY,
        ...
        'counts': {'usable': len(usable), 'usable_flagged': len(flagged), 'excluded': len(excluded), 'no_reference_blank_denver': len(doc['denver_blank_slices']),
        ...
        'limits': ['a usable pair means the photograph identity is settled by two criteria and the in-plane similarity residual is below 1 px; it says nothing about Denver label quality or anatomy',
