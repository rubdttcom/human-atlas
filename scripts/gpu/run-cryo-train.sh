...
"$PY/nnUNetv2_predict" -i "$ds/imagesTs" -o "$pred" -d "$D" -c 2d -f 0 \
    -tr nnUNetTrainer -p nnUNetPlans -chk checkpoint_final --disable_tta || rc=1
...
print('SENTINEL ' + json.dumps({'ok': rc == '0' and bool(res),
                                'variant': variant, 'dataset': name,
                                'checkpoint': res[0] if res else None,
