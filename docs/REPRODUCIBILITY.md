...
.venv/bin/python scripts/select-cryosection-pairs.py      # declared policy pairs-v1 -> generated/cryosection-pair-selection.json (usable / usable-flagged / excluded with ambiguity sets / blank); the RGB pilot reads this, never the transform's per-slice table directly
