...
.venv/bin/python scripts/build-cryo-ct-prior-block.py --block data/derived/nlm-vhf/cryosections/block2 --selftest   # 22 s: version 1 prior (TotalSegmentator total only, one class volume) -> ct-prior-tissue.nii.gz + generated/cryo-ct-prior-block2.json; bound to the two version 1 runs, do not change
.venv/bin/python scripts/build-cryo-ct-prior-v2-block.py --block data/derived/nlm-vhf/cryosections/block2 --selftest   # 130 s, 4 GB: version 2 prior for protocol v2 runs (registry/cryo-ct-prior-map-v2.json): consensus bone of TotalSegmentator + MOOSE + Skellytour, disagreement, in-plane distance to consensus bone, TotalSegmentator muscle -> ct-prior-v2-*.nii.gz + generated/cryo-ct-prior-v2-block2.json
.venv/bin/python scripts/build-cryo-nnunet-dataset.py --block data/derived/nlm-vhf/cryosections/block2 --variant rgb-plus-ct-prior --prior-version 2 --slices train   # 11 s: Dataset503_VHFCryoBlock2RGBPriorV2 (7 channels) + generated/cryo-nnunet-dataset-block2-rgb-plus-ct-prior-v2.json; --slices bands writes imagesTs; --prior-version 1 / rgb-only rebuild 502 / 501
.venv/bin/python scripts/test-record-nnunet-run.py        # 1 s: seed read-back of the run record (a literal, or a name resolved to its single integer assignment; cryo-entry.py writes SEED = 12345)
.venv/bin/python scripts/test-cryo-nnunet-dataset.py          # about 3 min: runs both builders into a temp tree and compares every channel and target against the sources; 27 cases including the version 2 prior
