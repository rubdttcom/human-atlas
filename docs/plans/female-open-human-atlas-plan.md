...
2. **Stage 0 of block C: trunk posture, fresh CT versus frozen block, measured 2026-09-13** (`generated/trunk-posture-offset.json`, published per mesh as `posture_offset` / `trunk_posture_context`): pelvis-anchor offset 3.35 mm of unidentified cause, about 2 mm relative to the pelvis through the thoracic and lumbar spine, about 5 mm at C1 to C3 and the skull; nothing corrected, nothing attributed. Next in stage 0: alignment check of the colour slices against Denver's aligned slices, then the RGB pilot in a Denver-covered region. Segmenter agreement does not correct registration or posture.
