...
2. **Stage 0 of block C: trunk posture, fresh CT versus frozen block, measured 2026-09-13** (`generated/trunk-posture-offset.json`, published per mesh as `posture_offset` / `trunk_posture_context`): pelvis-anchor offset 3.35 mm of unidentified cause, about 2 mm relative to the pelvis through the thoracic and lumbar spine, about 5 mm at C1 to C3 and the skull; nothing corrected, nothing attributed. Alignment of the colour photographs against Denver's aligned slices measured 2026-09-13 (`generated/cryosection-alignment-check.json`, `transforms/nlm-cryosection-to-vhf.json`): three rigid blocks, residual below 0.1 px on every slice, one Denver pixel = 2.0000 photograph pixels, 36 blank Denver slices, 1.0 mm of z removed by Denver at the knee gap; extrapolation above the pelvis marked unverified. Photograph-to-CT check measured 2026-09-13 (`generated/photo-ct-alignment-check.json`): above the pelvis the placed fresh CT and the photographs differ by a whole-trunk rotation of about 3.6 deg (42 mm at the head); the photographs there stay `extrapolated-unverified` with that uncertainty published. Correction of the same day: Denver's aligned CT is the NLM fresh CT resampled, so the trunk posture offset measured two placements of one acquisition, not posture (fields kept, renaming in the backlog). Stage 0 complete as measurement. Observability quarantine of 15 block-face slices (`registry/cryosection-quarantine.json`) applied before the identity policy in the pair selection. Next: the RGB pilot in the pelvis block from `generated/cryosection-pair-selection.json`. Segmenter agreement does not correct registration or posture.
3. **Lumbosacral `S1` dossier (backlog)** (`docs/plans/vhf-s1-lumbosacral-dossier.md`): cryosections, same-donor references (HRA skeleton, NIH 3D entry, Denver sacrum), nomenclature alternatives (sixth lumbar, lumbarised S1, transitional segment). Goal: resolve if the evidence allows; otherwise keep geometric ids and pending names. Does not block any other item.
4. **CT bone candidates (backlog; touched only as a prerequisite of block C).** Name agreement between models is the starting point, not the proof: per bone, the candidates must correspond geometrically (overlap on the CT grid), agree in laterality, be equivalent classes across the three label vocabularies, and have enough eligible models covering the region. Outputs are `machine-unverified` candidates with their disagreement kept and alternatives preserved; they are compared with the current source (`nlm-vhf-ct`; Denver where it exists) and never replace the composite by majority alone. Denver stays where it is used unless a documented comparison and a recorded decision replace it. Acceptance and substitution criteria: section 2.6 of plan B, fixed before the batch is run.
5. **Anatomical review (optional stage 8)** of what remains, only if the project gains attention: landmarks (viewer export -> `registry/landmark-review.json`), CT label boundaries, HRA placement, instance names. Not a gate for anything above.
