...
1. Download Denver's aligned whole-body CT and original label maps; run TotalSegmentator, MOOSE and Skellytour on that CT; compute the three-way consensus and compare with `nlm-vhf-ct` (Dice, centroid offsets per label). Deliverable: `generated/ct-prior-consensus.json` and the first measurement of the trunk posture offset. **Status 12 September 2026:** the three models ran on both CTs and the instance consensus exists on both (`generated/ct-vertebra-instances-{nlm,denver}.json`); the posture offset itself is not yet computed and is now the first item of stage 0, ahead of any use of CT labels as trunk guides. **Status 13 September 2026:** measured (`scripts/trunk-posture-offset.py`, `generated/trunk-posture-offset.json`): a common shift of 3.35 mm shared by every level and the pelvis anchor (the two pelvis registrations differ, not posture); relative to the pelvis about 2 mm posterior through the thoracic and lumbar spine, 2.5 to 4 mm at C4 to C7, about 5 mm at C1 to C3 and the skull; own-fit rotations below 0.5 deg except three instances whose residuals point to segmentation. Published in the provenance of every trunk CT mesh (`posture_offset`, `trunk_posture_context`). Nothing is corrected; the alignment check decides which registration carries the common shift.
