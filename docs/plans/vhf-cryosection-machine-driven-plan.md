...
"Fidelity equal to the anatomist" is therefore a measurement, not a promise. Section 2 defines it. Section 3.4 shows that the reference itself (Denver) published no inter-rater statistic. Human work in plan B is limited to two steps: the sealed test set (about 4 hours of drawing, section 2.4) and the blinded audit (section 2.3).
...
| Prior consensus | every structure with a CT prior | majority of the CT models **that know the class** (coverage matrix `registry/ct-prior-coverage.json`, model x structure); a model that has no class for a structure abstains and never votes background; where only one model knows the class the prior is single-source and is marked so. Disagreement is recorded as uncertainty. The models share training data and biases, so consensus is a consistency check, not an independent proof; the photographs and the audit remain the arbiters. |
