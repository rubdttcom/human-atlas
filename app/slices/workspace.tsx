...
export default function SlicesWorkspace({onExit,previousSource}:{onExit:()=>void;previousSource:string}){
  ...
  useEffect(()=>{
    ...
    (async()=>{
      ...
      const atlas={...source,parts:source.parts.filter(p=>ids.has(p.id))};
      if(!active)return;
  ...
  const move=useCallback((point:Vec3)=>{
    if(!loaded)return;const voxel=transform(loaded.volume.stageToVoxel,point);
    if(voxel.every((v,i)=>v>=-1e-6&&v<=loaded.volume.dimensions[i]-1+1e-6)){setPosition(point);setNotice('');}
  ...
  return <main className="slices-workspace">
    <header className="slices-heading"><div><span className="slices-kicker">FEMALE VHF · ORIGINAL CT</span><h1>Slices <small>Abdomen</small></h1></div><nav aria-label="Viewer mode"><button onClick={onExit}>Atlas</button><button aria-current="page">Slices</button></nav></header>
    <p className="source-transition">CT acquisition with its CT-derived structures. Your {previousSource} atlas state is preserved for return.</p>
    {!loaded||error?<section className="slices-message" role={error?'alert':'status'}><h2>{error?'Slices unavailable':'Loading abdominal preview…'}</h2><p>{error||'Checking source identity, image grid and file integrity.'}</p>{error&&<><p>Local preview: run <code>.venv/bin/python scripts/build-viewer-volume.py --serve</code> if the package is missing or the source atlas changed.</p><button onClick={()=>setRetry(n=>n+1)}>Retry preview</button></>}</section>:<>
      <div className="slices-body">
        <aside className="slices-controls" aria-label="Slice controls">
          ...
          <label>Click action<select value={clickAction} onChange={e=>setClickAction(e.target.value as 'navigate'|'select')}><option value="navigate">Move crosshair</option><option value="select">Select label</option></select></label>
          ...
          <label className="check"><input type="checkbox" checked={fill} onChange={e=>setFill(e.target.checked)}/>Fill mask (boundary is default)</label>
          <label className="check"><input type="checkbox" checked={showPlanes} onChange={e=>setShowPlanes(e.target.checked)}/>Show planes in 3D</label>
