...
export interface DiscrepancyThreshold {pelvis_anchor_offset_mm:number;model_range_mm:number;heuristic_threshold_mm:number;beyond_threshold:boolean;
 relative_anchor_disagreement_mm?:number;relative_heuristic_threshold_mm?:number;relative_beyond_threshold?:boolean;rule?:string}
...
export interface PostureOffset {status:string;report:string;report_sha256:string;frame?:string;match?:string;centroid_offset_vhf_mm?:OffsetVector;relative_to_pelvis?:OffsetVector|null;
 ...
 discrepancy_threshold?:DiscrepancyThreshold|null;registration_floor_pelvis_mm?:number;whole_spine_chain_rotation_deg?:number|null;note?:string}
...
export interface LinkedLevel {id:string;hra_name_by_order?:string|null;centroid_offset_vhf_mm:OffsetVector;relative_to_pelvis?:OffsetVector|null;own_rigid_fit_angle_deg:number;heuristic_threshold_mm?:number|null}
export interface TrunkPostureContext {report:string;report_sha256:string;frame?:string;link:string;instance_id:string;linked_instance_ids?:string[];linked_levels?:LinkedLevel[]|null;instance_hra_name_by_order?:string|null;mesh_centroid_z_vhf_mm:number;
 ...
 heuristic_threshold_mm?:number|null;registration_floor_pelvis_mm:number;whole_spine_chain_rotation_deg?:number|null;note?:string}
...
export function postureSection(record:ModelAgreement):AgreementSection|null {
 ...
 if(p){
  ...
  const u=p.discrepancy_threshold;
  const rows:Row[]=[
   ['Centroid offset (as placed)',vec(p.centroid_offset_vhf_mm)],
   ['Pelvis-anchor offset',p.common_shift_vhf_mm?`${vec(p.common_shift_vhf_mm)}: shared by every level; its cause (registration translation, angular registration error, anchor segmentation) is not identified by this calculation`:'not recorded'],
   ['Relative to the pelvis',p.relative_to_pelvis?`${vec(p.relative_to_pelvis)}: one translational reference removed; still a mixture of posture, angular registration error and segmentation, not separated`:'not recorded'],
   ['Own rigid fit',p.own_rigid_fit?`rotation ${num(p.own_rigid_fit.angle_deg,2,' deg')} (about x ${num(p.own_rigid_fit.about_x_right_deg,2)}, y ${num(p.own_rigid_fit.about_y_anterior_deg,2)}, z ${num(p.own_rigid_fit.about_z_superior_deg,2)}); residual after the fit p95 ${num(p.own_rigid_fit.residual_after_fit_p95_mm,2,' mm')} (segmentation differences plus discretisation, sampling and fit limits)`:'not recorded'],
   ['Surface distance as placed',num(p.surface_p95_placed_mm,2,' mm')+' p95, NLM surface to Denver surface'],
   ['Heuristic discrepancy threshold',u?`absolute ${num(u.heuristic_threshold_mm,2,' mm')} (pelvis-anchor offset ${num(u.pelvis_anchor_offset_mm,2,' mm')} + model range ${num(u.model_range_mm,2,' mm')}); relative ${num(u.relative_heuristic_threshold_mm,2,' mm')} (anchor disagreement ${num(u.relative_anchor_disagreement_mm,2,' mm')} + model range) · relative offset ${u.relative_beyond_threshold?'beyond':'within'} the heuristic threshold. A descriptive threshold, not an error bound or a confidence level; being beyond it attributes nothing to posture.`:'not recorded'],
 ...
 if(c){
  const rows:Row[]=[
   ['Level used',`${c.instance_id}${c.instance_hra_name_by_order?` (by-order candidate ${c.instance_hra_name_by_order}, name pending)`:''} · ${c.link}${c.linked_instance_ids&&c.linked_instance_ids.length>1?` · linked instances ${c.linked_instance_ids.join(', ')}`:''}`],
   ...(c.linked_levels&&c.linked_levels.length>1?c.linked_levels.map(l=>[`Linked ${l.id}`,`${l.hra_name_by_order?`by-order candidate ${l.hra_name_by_order} · `:''}relative to the pelvis ${vec(l.relative_to_pelvis)} · own fit ${num(l.own_rigid_fit_angle_deg,2,' deg')}`] as Row):[]),
   ...
   ['Pelvis-anchor offset',c.common_shift_vhf_mm?`${vec(c.common_shift_vhf_mm)}: shared by every level; cause not identified by this calculation`:'not recorded'],
   ['Relative to the pelvis',c.relative_to_pelvis?`${vec(c.relative_to_pelvis)}: one translational reference removed; posture, angular registration error and segmentation not separated`:'not recorded'],
   ['Own rigid fit of that level',num(c.own_rigid_fit_angle_deg,2,' deg')],
   ['Heuristic discrepancy threshold at that level',num(c.heuristic_threshold_mm,2,' mm')+' (pelvis-anchor offset + model range; descriptive, not an error bound)'],
